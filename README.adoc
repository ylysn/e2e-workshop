...
Please complete only one of the two following steps: `Step 1a` or `Step 1b`. Follow `Step 1a` if you have Git installed on your machine, else, follow `Step 1b`. +

== Step 1a. (Optional): Get github project
...
== Step 1b. (Optional): Download repository using GUI
...
____
Upload the file *Stocks_Intraday_Alpha_Template.json* (*`Note`*: You had downloaded this file in `Step 1a` or `Step 1b` depending on what you chose initiatlly.).
____
